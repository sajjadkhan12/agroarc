...
class FertilizerRequest(BaseModel):
    ...
    - Soil_Type: Must be a non-empty soil type string from trained categories
    - Crop_Type: Must be a non-empty crop type string from trained categories
    ...
    # Categorical variables (validated against encoder classes in route)
    Soil_Type: str = Field(
        ...,
        min_length=1,
        title="Soil Type",
        description="Soil type name (must exist in model-supported categories)"
    )
    Crop_Type: str = Field(
        ...,
        min_length=1,
        title="Crop Type",
        description="Crop type name (must exist in model-supported categories)"
    )
...
class GeneralChatRequest(BaseModel):
    """
    General chat request for backend AI assistance.
    """
    message: str = Field(
        ...,
        min_length=1,
...
class GeneralChatResponse(BaseModel):
    """
    General chat response generated by backend AI service.
    """
    reply: str = Field(
        ...,
